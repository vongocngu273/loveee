...
(function(d){d(document).ready(function(b){b("body").vegas({slides:[{src:CONFIG.background}],preloadImage:!0,overlay:!0});var c=b("#txt");c.typist({speed:10});c.typistPause(2E3);var d=CONFIG.content.length-1;CONFIG.content.map(function(a,g){c.typistAdd(a.text);g===d?c.typistPause(a.delay,function(){b("#buttons").fadeIn("fast")}):c.typistPause(a.delay)});var e=!1,f=b("#buttons");f.on("click","#btn-yes",function(a){swal(CONFIG.alert.success.title,CONFIG.alert.success.message,"success")}).on("click",
"#btn-no",function(a){swal(CONFIG.alert.error.title,CONFIG.alert.error.message,"error")});CONFIG.alert.troll&&(f.on("mouseenter",function(a){e||(e=!0,b(document).mousemove(function(a){b("#btn-no").css({position:"fixed",top:a.offsetY+10,left:a.offsetX+10})}))}),b(document).on("keydown keyup",function(a){var b=a.which||a.keyCode;if(9===b||123===b)return a.preventDefault(),!1}),b("#btn-no").on("focus",function(a){a.preventDefault();b(this).blur()}))})})(jQuery);
